...
var _ = Describe("GoPool Test", func() {
	...
	Describe("Given we can create an instance of gopool", func() {
		...
		Context("if we create an instance of gopool and create multiple jobs with even sync jobs and call request for shutdown", func() {
			It("then we should execture all of them sucessfully", func() {
				gp := NewGoPool(context.Background())
				Expect(gp).ShouldNot(BeNil())
				fn1 := func(ctx context.Context, args ...interface{}) error {
					gp.RequestShutDown()
					return nil
				}
				fn := func(ctx context.Context, args ...interface{}) error {
				...
				gp.AddSyncJob("test3", fn, context.Background(), context.Background(), 5)
				time.Sleep(time.Second)
				gp.AddJob("request shutdown", fn1, context.Background(), context.Background(), 5)
				gp.ShutDown(true, time.Second)
			})
		})
	})
